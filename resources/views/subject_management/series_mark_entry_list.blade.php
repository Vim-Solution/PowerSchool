        ...
        <div class="table-responsive">
            <table id="@lang('general.data_table')" class="table table-striped">
                ...
                <tr>
                    ...
                    <th>@lang('subject_management/manage_test.series')</th>
                ...
                @foreach($students as $student)
                    <tr>
                        ...
                        <td>{{ $seriesData->where(trans('database/table.matricule'),$student->matricule)->first()->series_series_code  }}</td>
                        <td>
                            <div class="form-group" style="background-color: white">
                                <input type="text" class="form-control" name="{{ $student->matricule }}"
                                       id="{{ $student->matricule }}"
                                       value="{{ $testScores->where(trans('database/table.students_student_id'),$student->student_id)->isEmpty() ? 0.0 :$testScores->where(trans('database/table.students_student_id'),$student->student_id)->first()->test_score }}">
                            </div>
                        </td>
                    </tr>
                @endforeach
